...
def user_move(board):
    user_position = 0
    while (1):
        # Loop over user input until we receive a valid input
        try:
...
def minimax(board, is_maximizing_player, depth = 0):
    # Get current board state
    eval = evaluate_board(board)
    # Check if this is a terminal (win/lose/draw) state
    if eval != None:
        # If it is, we return the evaluation
        # return eval 
    ...
    for i in range(len(board)):
        for j in range(len(board)):
            ...
            if board[i][j] == ' ':
                ...
                if is_maximizing_player:
                    # Set the current index to the player icon
                    child[i][j] = ai
                    # This line will recursively call the minimax function until we get a terminal state
                    eval = minimax(child, False, depth+1) + heuristic(child)
                    # Find the path that maximizes the evaluation.
                    best_eval = max(best_eval, eval)
                else:
                    # Same as above but,
                    child[i][j] = user
                    eval = minimax(child, True, depth+1) + heuristic(child)
                    # this player wants to minimize the evaluation.
...
def display_board(board):
    # Utility function to display the board, and final winner/draw
    scores = {
        -10: "User",
        10: "AI",
    }
